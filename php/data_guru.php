...
<body class="bg-[#fcfcfd] text-gray-800 font-sans antialiased selection:bg-primary selection:text-white flex flex-col min-h-screen">
    ...
    <nav class="bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100 shadow-[0_4px_30px_rgba(0,0,0,0.02)]">
                    ...
                    <div class="relative ml-4" id="userMenuContainer">
                        ...
                        <div id="userMenu" class="hidden absolute right-0 mt-3 w-48 bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-gray-100 py-2 z-50 transform origin-top-right transition-all">
                            <?php if (isset($_SESSION["login"])) : ?>
                                <a href="#" onclick="openModal('ganti_pw')" class="block px-5 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-primary transition-colors font-medium" <?= $admin; ?>><i class="bi bi-key me-2"></i>Ganti Password</a>
        ...
        <div id="mobileMenu" class="hidden md:hidden bg-white border-t border-gray-100 px-4 pt-4 pb-6 space-y-2 shadow-xl">
            ...
            <?php if (isset($_SESSION["login"])) : ?>
                ...
                <a href="#" onclick="openModal('ganti_pw')" class="block px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 font-medium" <?= $admin; ?>><i class="bi bi-key me-2"></i>Ganti Password</a>
    ...
    <main class="flex-grow pt-12 pb-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <!-- Profil Guru Card -->
            <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 overflow-hidden max-w-4xl mx-auto">
                <div class="bg-gradient-to-r from-secondary to-primary px-8 py-6 relative overflow-hidden">
                    <h1 class="font-serif text-2xl font-bold text-white relative z-10 flex items-center gap-3"><i class="bi bi-person-badge"></i> Data Guru</h1>
                </div>
                
                <div class="p-8">
                    <div class="bg-gray-50 rounded-2xl p-8 border border-gray-100 relative overflow-hidden">
                        <!-- Decorative element -->
                        <div class="absolute -right-12 -top-12 text-gray-200/50 opacity-20 pointer-events-none">
                            <i class="bi bi-person-circle" style="font-size: 15rem;"></i>
                        </div>
                        
                        <div class="relative z-10">
                            <dl class="grid grid-cols-1 gap-y-6 text-sm sm:text-base">
                                <div class="grid grid-cols-3 gap-6 items-center border-b border-gray-200/60 pb-4">
                                    <dt class="font-bold text-gray-500 col-span-1">Nama</dt>
                                    <dd class="font-bold text-gray-900 col-span-2 text-lg"><?= $guru["nama_guru"]; ?></dd>
                                </div>
                                <div class="grid grid-cols-3 gap-6 items-center border-b border-gray-200/60 pb-4">
                                    <dt class="font-bold text-gray-500 col-span-1">NIP / NUPTK</dt>
                                    <dd class="font-medium text-gray-800 col-span-2 bg-white px-4 py-2 rounded-lg inline-block shadow-sm border border-gray-100">
                                        <?= $guru["nip"]; ?>
                                    </dd>
                                </div>
                                <div class="grid grid-cols-3 gap-6 items-center">
                                    <dt class="font-bold text-gray-500 col-span-1">Email</dt>
                                    <dd class="font-medium text-gray-800 col-span-2 flex items-center gap-2">
                                        <i class="bi bi-envelope-fill text-gray-400"></i> <?= $guru["email"]; ?>
                                    </dd>
                                </div>
                            </dl>
                        </div>
