                ...
                <div class="p-8">
                    <form action="" method="post" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Kelas -->
                            <div>
                                <label for="kelas" class="block text-sm font-bold text-gray-700 mb-2">Kelas <span class="text-xs font-normal text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full ml-2 border border-amber-100">(wajib isi ulang)</span></label>
                                <select name="kelas" id="kelas" class="w-full rounded-xl border-gray-200 shadow-sm focus:border-primary focus:ring-primary text-sm px-4 py-2.5 border outline-none transition-all bg-white" required>
                            ...
                            <!-- Jurusan -->
                            <div>
                                <label for="jurusan" class="block text-sm font-bold text-gray-700 mb-2">Jurusan <span class="text-xs font-normal text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full ml-2 border border-amber-100">(wajib isi ulang)</span></label>
                                <select name="jurusan" id="jurusan" class="w-full rounded-xl border-gray-200 shadow-sm focus:border-primary focus:ring-primary text-sm px-4 py-2.5 border outline-none transition-all bg-white" required>
                        ...
                        <!-- NIS -->
                        <div>
                            <label for="nis" class="block text-sm font-bold text-gray-700 mb-2">NIS</label>
                            <input type="number" id="nis" name="nis" class="w-full rounded-xl border-gray-200 shadow-sm focus:border-primary focus:ring-primary text-sm px-4 py-2.5 border outline-none transition-all" required autocomplete="off" value="<?= $siswa["nis"]; ?>" placeholder="5 digit">
                        </div>
                        
                        <!-- Nama Lengkap -->
                        <div>
                            <label for="nama" class="block text-sm font-bold text-gray-700 mb-2">Nama Lengkap</label>
                            <input type="text" id="nama" name="nama" class="w-full rounded-xl border-gray-200 shadow-sm focus:border-primary focus:ring-primary text-sm px-4 py-2.5 border outline-none transition-all" required autocomplete="off" value="<?= $siswa["nama_siswa"]; ?>" placeholder="Nama lengkap siswa">
                        </div>
                        ...
                        <!-- Foto -->
                        <div>
                            <label for="foto" class="block text-sm font-bold text-gray-700 mb-2">Foto Profil</label>
                            <div class="flex items-center gap-6">
                                <div class="w-16 h-16 rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex-shrink-0 bg-gray-50 flex items-center justify-center">
                                    <?php if (empty($siswa["foto"])) : ?>
                                        <img src="../../img/logosmk12.png" alt="Profile" class="w-full h-full object-contain p-2">
                                    <?php else : ?>
                                        <img src="../../foto_siswa/<?= $siswa["foto"]; ?>" alt="Profile" class="w-full h-full object-cover">
                                    <?php endif; ?>
                                </div>
                                <input type="file" id="foto" name="foto" class="w-full text-sm text-gray-600 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 cursor-pointer focus:outline-none transition-all">
                            </div>
                        </div>
                        
                        <div class="pt-6 border-t border-gray-100 flex flex-col sm:flex-row gap-3 justify-end">
                            <a href="../data_siswa.php?id=<?= $id; ?>" class="inline-flex justify-center items-center px-6 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold hover:bg-gray-50 transition-colors focus:ring-2 focus:ring-gray-200 focus:ring-offset-2">Batal</a>
